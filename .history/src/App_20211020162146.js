...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${url}?_start=0&_end=2`)
      ...
      .then((result) => {
        result = result.map(obj => ({...obj, timeStamp: `${new Date.toLocaleTimeString('vi-VN')} - ${new Date.toLocaleDateString('vi-VN')}`}))
        setList(result);
        setIsLoading(false);
        console.log(result);
      });
  ...
  useEffect(() => {
    if (isAddding) {
      fetch(`${url}/${row}`)
        .then((response) => response.json())
        .then((result) => {
          result = {...result, timeStamp: `${new Date.toLocaleTimeString('vi-VN')} - ${new Date.toLocaleDateString('vi-VN')}`};
          setIsLoading(false);
          setIsAddding(false);
          list.push(result);
          console.log(result);
